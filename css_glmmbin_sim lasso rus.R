...
reps <- 1 # change to 200

j_vals <- c(10000) # number of clusters for n=60,000 (creating ground truth)
i_vals <- c(6) # cluster size (intact for population data generation)
int_vals <- c(-3) # individual-level imbalance controlled by the intercept
tau_vals <- c(0)
p <- 20
...
### 3b. cluster-sensitive random undersampling (RUS) for the k-fold CV

  # create empty data frame for undersampled data
    train_data_undersampled <- data.frame()
  ...
  for (j in training_clusters) {
    ...
    # undersample only if both classes exist
    if (nrow(cases_0) > 0 && nrow(cases_1) > 0) {
      # undersample only if there are more 0 cases than 1s
      if(nrow(cases_0) >  nrow(cases_1)) {
      undersampled_cases <- cases_0[
        sample(nrow(cases_0), nrow(cases_1), replace = FALSE),
        ]
        balanced_data <- rbind(undersampled_cases, cases_1)
      } else if (nrow(cases_1) >  nrow(cases_0)) {
        undersampled_cases <- cases_1[
          sample(nrow(cases_1), nrow(cases_0), replace = FALSE),
        ]
        balanced_data <- rbind(cases_0, undersampled_cases)
      } else {
    ...
    train_data_undersampled <- rbind(train_data_undersampled, balanced_data)
  } # end of cluster loop
...
### 3c. hyperparameter tuning for glmmlasso for each fold of the k-fold cross-validation
  ...
  train_data_undersampled$y <- as.numeric(as.character(train_data_undersampled$y))
  validation_data$y <- as.numeric(as.character(validation_data$y))
  
  # convert cluster variable to factor in this fold
  train_data_undersampled$clust <- as.factor(train_data_undersampled$clust)
  ...
  if (i == 1) {
    lambdas <- cv.glmmLasso:::buildLambdas(fix = fix, rnd = list(clust = ~1),
                                           data = train_data_undersampled, nlambdas = 10)
  ...
  # predictionMatrix <- cv.glmmLasso:::predict.glmmLasso_MultLambdas(
  #   object = modList_fold,
...
### 4b. cluster-sensitive random undersampling in the full training dataset

# create empty data frame for undersampled data
train_data_unders_full <- data.frame()
...
for (j in training_clusters_full) {
  ...
  if (nrow(cases_0_full) > 0 && nrow(cases_1_full) > 0) {
    ...
    if(nrow(cases_0_full) >  nrow(cases_1_full)) {
      undersampled_cases_full <- cases_0_full[
        sample(nrow(cases_0_full), nrow(cases_1_full), replace = FALSE),
      ]
      balanced_data_full <- rbind(undersampled_cases_full, cases_1_full)
    } else if (nrow(cases_1_full) >  nrow(cases_0_full)) {
      undersampled_cases_full <- cases_1_full[
        sample(nrow(cases_1_full), nrow(cases_0_full), replace = FALSE),
      ]
      balanced_data_full <- rbind(cases_0_full, undersampled_cases_full)
    } else {
  ...
  train_data_unders_full <- rbind(train_data_unders_full, balanced_data_full)
} # end of cluster loop

# convert y to numeric and clust to factor in final training data
train_data_unders_full$y <- as.numeric(as.character(train_data_unders_full$y))
train_data_unders_full$clust <- as.factor(train_data_unders_full$clust)

### 4c. glmmLasso on full undersampled training data with optimal lambda

# using function created to fix chosen lambdas issue
final_model <- fit_path(fix = fix, rnd = list(clust = ~1),
                        data = train_data_unders_full,
                        family = binomial(link = "logit"),
...
saveRDS(
  results,
  file = file.path(save_dir, "results_lasso_rus_tau2.RDS")
)

# save results as .csv
write.csv(results, paste0(save_dir,"/results_lasso_rus_tau2.csv"), row.names = FALSE)